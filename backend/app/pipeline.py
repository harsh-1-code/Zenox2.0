...
def assess(
    ...
    return Verdict(
        **a.model_dump(),
        session_id=sid,
        # Built deterministically, then said in the person's language if it is one we
        # did not hand-write. The steps never change - only the words.
        actions=localize.actions_into(actions.build(a, lang), a.reply_lang),
